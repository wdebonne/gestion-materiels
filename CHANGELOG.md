...
- **Le redéploiement Portainer s'arrêtait sur « pull access denied ».** L'image de l'application n'est publiée sur aucun registre : elle se construit depuis le `Dockerfile` du dépôt. Or Portainer lance `docker compose pull` avant de redéployer — le GitOps aussi, à chaque commit — et Compose partait chercher `gestion-materiels-app` sur Docker Hub. Les deux piles déclarent désormais `pull_policy: build`, que Compose lit comme « cette image se construit ici » et saute. Le guide Portainer ne fait plus cocher « Re-pull image », et donne l'erreur en dépannage
