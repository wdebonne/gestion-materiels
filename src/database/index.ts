...
class DatabaseManager {
  ...
  private async createTables(migrationsAuto = true): Promise<void> {
    ...
    const tables = [
      ...
      `CREATE TABLE IF NOT EXISTS green_space_elements (
        ...
        custom_fields ${textType} DEFAULT ('{}'),
        area_source VARCHAR(20) DEFAULT 'saisi',
        exclude_from_costs ${boolType} DEFAULT 0,
        created_at DATETIME ${timestampDefault},
        updated_at DATETIME ${timestampDefault},
  ...
  private async createIndexes(): Promise<void> {
    const indexes: Array<[string, string, string]> = [
      ...
      // Le plan d'un espace lit toutes ses annotations d'un coup : sans index,
      // le balayage grandit avec le nombre total de repères de la commune.
      ['idx_gs_annotations_space', 'green_space_annotations', 'green_space_id'],
      ['idx_gs_groups_space', 'green_space_groups', 'green_space_id'],
