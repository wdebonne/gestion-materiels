...
const EXEMPTIONS: Record<string, string> = {
  ...
  'services/manifestationSorties.service.ts':
    "Ne rend que le mouvement d'un matériel sur une manifestation — nom, quantité sortie, dates — et applique une portée : celle des manifestations (filtreManifestations), qui est celle de l'écran. Un compte qui voit une manifestation voit déjà son matériel par GET /manifestations/:id ; lui appliquer en plus la portée par catégorie lui cacherait la moitié du chargement d'un camion qu'il est chargé de préparer. Aucune fiche de matériel, aucun prix d'achat.",
  'services/tourneeManifestation.service.ts':
    "Même situation que manifestationSorties : ne rend que ce qu'une manifestation emporte — nom, repère, quantités sorties et rendues — et applique la portée des manifestations (filtreManifestations), qui est celle de l'écran. Un agent chargé de préparer un camion doit voir tout le chargement ; lui cacher les lignes dont la catégorie ne lui est pas ouverte lui ferait rapporter la moitié des chaises et laisser l'autre dehors. Aucune fiche de matériel, aucun prix d'achat.",
  'database/migrations/026_entites_html_importees.ts':
    "Migration de réparation, jouée au démarrage du serveur : il n'y a ni requête ni utilisateur, donc aucune portée à appliquer. Ne lit `objects` que pour décoder sur place les entités HTML de `name` et `reference` laissées par une première version de l'import Snipe-IT, et ne rend rien à personne.",
  'services/snipeIt.service.ts':
    "Reprise d'un inventaire tenu dans un autre système, réservée à l'administrateur par `snipeIt.routes.ts` — qui applique `requireAdmin` sur tout le routeur, et pour qui `getAccessibleCategoryIds` rend `null`, c'est-à-dire aucune restriction. Les lectures de `objects` ne servent qu'à retrouver ce qui a déjà été importé et à refuser un numéro d'inventaire déjà pris ; elles ne rendent aucune fiche à l'appelant. Filtrer par catégorie ici laisserait au contraire créer un doublon d'un matériel qu'on ne voit pas, ce qui est précisément ce que la table de correspondance existe pour empêcher.",
  'services/cles.service.ts':
    "Ne lit `objects` que pour nommer ce qui est déjà rattaché à un matériel dont l'accès vient d'être vérifié : la composition d'un trousseau, les trousseaux qui contiennent une clé, l'objet désigné par un jeton. Toutes les entrées du module passent par cle.routes.ts, qui applique filtreObjets sur les listes et peutVoirObjet sur chaque identifiant, ou par clePublic.routes.ts, qui vérifie la catégorie avant de rendre le moindre détail — un jeton seul ne donne que le message de restitution. Appliquer en plus la portée ici cacherait à un agent la moitié des clés d'un trousseau qu'il a le droit de consulter, et il en rendrait la moitié.",
  'services/ticketNotify.service.ts':
    "Ne joint `objects` que pour nommer le matériel dans le courriel d'une demande qui le désigne déjà, et n'est jamais piloté par quelqu'un qui navigue dans le parc : la fonction tourne côté serveur, après une action, sur les seuls destinataires concernés — demandeur, technicien, service destinataire, observateurs, et les personnes qu'un administrateur a explicitement désignées pour cette catégorie ou ce bâtiment. Filtrer par catégorie ici écrirait « Matériel hors de votre périmètre » dans un ordre d'intervention, c'est-à-dire exactement le renseignement dont son destinataire a besoin pour se déplacer — le même raisonnement que pour manifestationSorties. Aucune liste de parc, aucun prix d'achat, aucune fiche.",
  'services/ticketsReferentiel.service.ts':
    "Ne joint `objects` que dans `materielsDe`, qui rend le matériel **explicitement attribué** à une personne — son téléphone, son ordinateur — et à elle seule, puisque la requête part de `user_materiels` filtrée sur son propre identifiant. Appliquer la portée par catégorie y serait un contresens : elle cacherait à quelqu'un son propre poste au motif que la catégorie Informatique ne lui est pas ouverte en consultation, alors qu'un administrateur lui a justement affecté ce matériel. Ne rend que nom, référence et emplacement, jamais de prix d'achat ni de fiche, et jamais un matériel qui n'est pas le sien.",
  'routes/ticketReferentiel.routes.ts':
    "Tout le routeur est gardé par `requireAdmin` sur les routes qui lisent `objects` — le détail des rattachements d'une personne — et `getAccessibleCategoryIds` rend `null` pour un administrateur, c'est-à-dire aucune restriction à appliquer. La jointure ne sert qu'à nommer les matériels déjà attribués, pour que l'écran d'attribution affiche « Ordinateur portable (INF-042) » plutôt qu'un identifiant nu.",
